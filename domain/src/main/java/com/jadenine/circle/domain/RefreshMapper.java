 ...
 */
class RefreshMapper<E extends Savable, D extends Updatable<E>> implements Func1<JSONListWrapper<E>, List<D>> {
    ...
    public List<D> call(JSONListWrapper<E> entities) {
        RestResultClassifier restResultClassifier = new RestResultClassifier(entities,
        ...
        if (needClear) {
            List<D> keptTopics = new ArrayList<>(source.subList(0, Math.min(source.size(), mapperDelegate
                    .getCapability())));
            source.clear();
